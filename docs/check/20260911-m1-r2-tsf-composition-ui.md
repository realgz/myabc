...
- [x] 用户完成 §3 核心场景（`nihao` 候选框 + 正确整句转换）并确认可用（2026-09-11）。
      排障过程发现并处理了一个僵尸引擎进程掩盖 CWD 修复的问题，见 debt-log。
- [ ] 其余场景（翻页、数字选字、退格、Esc、Shift 中英切换、标点、M1-12 杀引擎降级）
      未逐条要求用户单独复述确认；用户已示意继续推进，按"核心链路验证通过、细节问题
      后续发现随时修"处理，不阻塞打 tag。
- [x] 打 tag `m1-quanpin`（见提交历史）。
