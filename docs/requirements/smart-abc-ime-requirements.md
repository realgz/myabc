...
- 状态：生效
- 更新日期：2026-09-13（§3.3/§4 更新：新增输入方案切换，五笔从"预留不实现"变为
  "实现"）
- 来源：用户对话（2026-09-09；2026-09-13 补充） + deep-planner 规划
  （`docs/plan/08-wubi-input-scheme-plan.md`）
...
### 3.3 输入方案切换（新增，2026-09-13）
- 三种可选输入方案：**智能ABC**（默认，§3.1/§3.2 的全部经典特性）、**普通拼音**
  （同一个 libpinyin 引擎，但数字键任何时候直接选字、空格任何时候立即选中候选[0]，
  不做两段式确认——两段式确认是智能ABC 专属，因为它需要数字键身兼笔形码/数字
  续写与选字两种含义，普通拼音不存在这种复用冲突）、**五笔字型**（86 版，从零
  实现，数据来自开源码表，见 `assets/README.md`；支持重码数字键直选）。
- 切换方式：热键（默认 `Ctrl+Shift+W`，三态循环）；语言栏可见状态指示（文字按钮，
  点击同样触发切换）。
- 方案选择持久化：跨引擎重启/空闲自退出仍保留上次选择（小状态文件，非完整
  配置文件解析）。
- 五笔不含拆字算法（数据源已提供预算好的字/词->编码映射）、不含 98 版、不含
  字根图/前缀联想 UI、不含五笔专属自学习。
- 完整设计见 `docs/plan/08-wubi-input-scheme-plan.md`（§10 为最终权威版本）与
...
- 云输入 / 大模型联想
- 双拼方案（架构预留，不实现；跟 §3.3 的"普通拼音"是两回事——双拼是拼音输入
  的一种按键编码方式，属于 `InputConfig::scheme` 的 `shuangpin`（预留字段）
  范畴，跟 §3.3 新增的"方案大类"维度`InputConfig::method` 无关。五笔已于
  2026-09-13 从本条移出并实现，见 §3.3）
- 手写 / 语音
- ARM64（架构预留）
- 皮肤系统（候选窗先做够用的默认样式）
- 安装包签名与商店上架
